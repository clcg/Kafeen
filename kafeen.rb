...
require_relative File.join('lib', 'logic_tracker.rb')
require 'logger'
require 'open3'

cmd = Command.new(out_file_prefix: FILE_PREFIX, log_level: 'info')
...
cmd.genes2regions(genes_file: F_IN,
                  ...
                  intmed_output: CONFIG['intmed_output'])
...
if ['vep', 'both'].include?(annotator)
  cmd.add_vep(vcf_file: cmd.add_genes_result,
             ...
             vep_tags: CONFIG['third_party']['vep']['tags'],
             intmed_output: CONFIG['intmed_output'])
end
...
cmd.finalize_pathogenicity(vcf_file: cmd.add_predictions_result,
                           ...
                           intmed_output: CONFIG['intmed_output'],
                           maf_threshold: CONFIG['maf_thresh'])
