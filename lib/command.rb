...
class Command
  ...
  def regions2variants(bed_file:, vcf_files:, out_file_prefix:, keep_tmp_files: false)
    ...
    vcf_files.each do |key, vcf|
      if vcf['include'] or vcf['include'].nil? # RJM: checks flag in config.yml if the vcf source should be included or not in the kafeen run. true allows the source to be considered as normal, false ignores the source
